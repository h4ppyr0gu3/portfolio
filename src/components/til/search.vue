<template>
  <form @submit.prevent="handleSearch" class="mx-4 flex justify-center items-center mb-2">
    ...
    <div class="relative w-96 justify-center">
      ...
      <input type="search" v-model="searchText" name="search" id="blogSearch" class="block p-4 pl-10 text-sm text-gray-900 border-b border-gray-300 bg-transparent dark:border-gray-600 dark:placeholder-gray-400 dark:text-white border-b focus:outline-none dark:focus:border-sky-400 focus:border-sky-500 w-full" placeholder="Search for a Blog...">
      <button type="submit" class="text-white absolute right-2.5 bottom-2.5 bg-sky-500 hover:bg-sky-700 font-medium rounded-lg text-sm px-4 py-2 dark:bg-sky-600 dark:hover:bg-sky-700 " @click="handleSearch">Search</button>
    </div>
  </form>
  <List :til=til />
</template>
...
export default defineComponent({
  ...
  methods: {
    handleSearch() {
      ...
      // Your form handling logic here, using this.searchText.value
    }
  },
  data() {
